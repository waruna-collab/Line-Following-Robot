...
// This switches the internal gates of the CD74HC4067 chip
int readMux(int channel) {
  ...
  delayMicroseconds(10); // Give the multiplexer a tiny microsecond to stabilize
...
void calibrateSensors() {
  ...
  while (millis() - startTime < 2500) { 
    for (int i = 0; i < NUM_SENSORS; i++) {
      int val = readMux(i); // Read from the Multiplexer instead of a direct pin
...
void loop() {
  ...
  for (int i = 0; i < NUM_SENSORS; i++) {
    int val = readMux(i); // Fetch the value through the Mux!
  ...
  if (millis() - lastSendTime > 200) {
    // Line 1: Raw Analog sensor view (Now all 8 sensors without the gap!)
    SerialBT.println("Val: [ " + sensorString + "]");
    
    // Line 2: The exact Threshold for each sensor
    SerialBT.println("Thr: [ " + threshString + "]");
    
    // Line 3: The Tuning Variables
